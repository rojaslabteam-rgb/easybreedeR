...
Interactive pedigree visualization is implemented using the `visNetwork` package [@Almende2024]. Inbreeding coefficients ($F$) are calculated using the tabular method from the `inbreeding()` function in the `pedigreeTools` package [@Boerner2015]. Relationship analysis uses the `kinship()` function and graph-based traversal methods via the `igraph` package [@Csardi2006] to efficiently identify relationships. These capabilities ensure data integrity and facilitate an intuitive understanding of population structure.
