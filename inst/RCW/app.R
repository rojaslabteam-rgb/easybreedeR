...
scan_workspace <- function(workspace_paths = c(.rcw_workspace, .rcw_external_folders)) {
  ...
  for (workspace_path in workspace_paths) {
    if (!dir.exists(workspace_path)) next
    ...
    for (file_path in all_files) {
      rel_path <- gsub(paste0("^", gsub("\\\\", "/", workspace_path), "/?"), "", gsub("\\\\", "/", file_path))
    ...
    for (dir_path in c(workspace_path, all_dirs)) {
      rel_dir <- gsub(paste0("^", gsub("\\\\", "/", workspace_path), "/?"), "", gsub("\\\\", "/", dir_path))
      folder_label <- if (identical(rel_dir, "")) "root" else paste0("root", "/", rel_dir)
...
ui <- page_fillable(
  ...
  .canvas-node.selected.node-shape-diamond::before { border-color: #B89D5D; box-shadow: 0 6px 20px rgba(184,157,93,0.5); }\
    ...
    $(document).ready(function() {
      ...
      $('#canvas').droppable({
        ...
        drop: function(event, ui) {
          ...
          createCanvasNode(nodeId, filePath, fileName, x, y);
    ...
    function createCanvasNode(id, filePath, fileName, x, y) {
      var node = $('<div>')
        .attr('id', id)
        ...
        .html(
          ...
          '<div class=\"node-info\">' + filePath.substring(0, 30) + '...</div>'
        );

      $('#canvas').append(node);
      var defaultAppearance = { shape: 'square', color: '#ffffff' };
      canvasNodes[id] = { filePath: filePath, fileName: fileName, x: x, y: y, appearance: defaultAppearance };
  ...
  try { setNodeEndpoints(id, defaultAppearance.shape); } catch(e){}
    ...
    function showContextMenu(x, y, nodeId, filePath, fileName) {
      ...
      menu.html(
        ...
        '  Shape: Square' +
        '</div>' +
        '<div class=\"context-menu-item\" data-action=\"shape-diamond\">' +
        '  Shape: Diamond' +
        '</div>' +
        '<div class=\"context-menu-item\" data-action=\"color-red\">' +
        '  Color: Red' +
        '</div>' +
        '<div class=\"context-menu-item\" data-action=\"color-green\">' +
        '  Color: Green' +
        '</div>' +
        '<div class=\"context-menu-item\" data-action=\"color-blue\">' +
        '  Color: Blue' +
        '</div>' +
        '<div class=\"context-menu-item\" data-action=\"color-yellow\">' +
        '  Color: Yellow' +
        '</div>'
      );
    ...
    function handleContextMenuAction(action) {
      ...
      } else if (action === 'shape-square' || action === 'shape-diamond' || action.indexOf('color-') === 0) {
        var ap = {};
        if (action === 'shape-square') ap.shape = 'square';
        if (action === 'shape-diamond') ap.shape = 'diamond';
        if (action === 'color-red') ap.color = '#ffdddd';
        if (action === 'color-green') ap.color = '#ddffdd';
        if (action === 'color-blue') ap.color = '#dde7ff';
        if (action === 'color-yellow') ap.color = '#fff6cc';
        try { applyNodeAppearance(currentContextNode.id, ap); } catch(e) {}
        try { Shiny.setInputValue('node_set_appearance', { id: currentContextNode.id, color: ap.color, shape: ap.shape }, {priority: 'event'}); } catch(e) {}
      }
    }
    ...
    Shiny.addCustomMessageHandler('restore_canvas', function(data) {
      ...
      data.nodes.forEach(function(n) {
        createCanvasNode(n.id, n.filePath, n.fileName, n.x, n.y);
      });
      setTimeout(function() {
...
server <- function(input, output, session) {
  ...
  observeEvent(input$node_created, {
    ...
    rv$nodes[[nc$id]]$appearance <- list(shape = 'square', color = '#ffffff')
  ...
  observeEvent(input$node_set_appearance, {
    ...
    # msg is expected to contain id and either color and/or shape
  ...
  output$rmd_preview <- renderText({
    ...
    # Convert ord to a temporary nodes map keeping same ids
    tmp_nodes <- setNames(lapply(ord, function(x) x$node), sapply(ord, function(x) x$id))
    rmd <- tryCatch({
