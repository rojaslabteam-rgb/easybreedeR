...
get_label_local <- function(key, lang = "en") {
  ...
  effective_key <- if (startsWith(key, "datapreviewer_")) key else paste0("datapreviewer_", key)
  result <- try(gl(effective_key, lang, app = "datapreviewer"), silent = TRUE)
  if (inherits(result, "try-error")) {
    # Fallback to direct key lookup (for non-prefixed/global keys)
    result <- try(gl(key, lang, app = "datapreviewer"), silent = TRUE)
...
server <- function(input, output, session) {
  ...
  current_lang <- reactive({
    ...
    map_suite_lang_for_app(resolved, app = "datapreviewer")
  })
  
  # App title (use unified translation keys)
  output$appTitle <- renderText({ get_label("datapreviewer_app_name", current_lang()) })
  output$appSubtitle <- renderText({ get_label("datapreviewer_app_subtitle", current_lang()) })
  ...
  observeEvent(input$showDownloadModal, {
    ...
    showModal(modalDialog(
      ...
      div(style = "text-align: center;",
          h5(get_label("missing_value_modal_text", current_lang())),
          br(),
